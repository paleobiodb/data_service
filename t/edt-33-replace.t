...
#
# edt-33-replace.t : Test the operation of the 'replace_records' method.
# 
...
subtest 'basic' => sub {
    ...
    # Now try some replaces. Check for E_NO_KEY if no key is given.
    
    $edt = $T->new_edt($perm_a);
    
    $result = $edt->replace_record('EDT_TEST', { string_req => 'test a1' });
    
    ok( ! $result, "replace_record returned false with test_no missing" );
    $T->ok_has_error( qr/E_NO_KEY/, "found condition E_NO_KEY" );
    
    $T->ok_found_record('EDT_AUX', "name = 'ghi'", "found record with name 'ghi'");
    
    $result = $edt->replace_record('EDT_TEST', { string_req => 'test a2', test_no => '9999' });

    ok( ! $result, "replace_record returned false with test_no not found" );
    $T->ok_has_error( qr/E_PERM/, "found condition E_PERM" );
    
    # Now try an replace transaction, with record keys but without the CREATE allowance (to check
    # that it is not required).
    
    $edt = $T->new_edt($perm_a, { CREATE => 0 });
    
    $result = $edt->replace_record('EDT_TEST', { test_no => $test_keys[0], string_req => 'test x1', _label => 'x1' }); 
    
    ok( $result, "first replace was valid" ) || $T->diag_errors('latest');
    
    $result = $edt->replace_record('EDT_AUX', { aux_no => $aux_keys[1], name => 'replaced a', test_no => $test_keys[1] });
    
    ok( $result, "second replace was valid, setting test_no to the value it previously had" ) ||
	$T->diag_errors('latest');
    
    $result = $edt->replace_record('EDT_AUX', { aux_no => $aux_keys[2], name => 'replaced b' });
    
    ok( $result, "third replace was valid" ) || $T->diag_errors('latest');
    
    $T->ok_result( $edt->execute, "replace executed successfully" );
    
    my ($count1) = $edt->dbh->selectrow_array("
		SELECT count(*) FROM $TABLE{EDT_AUX} WHERE name = 'replaced a'");

    is( $count1, 1, "found record with updated name" );    
    
    $T->ok_found_record('EDT_AUX', "name = 'replaced a'", "found record with replaced name");
    ...
    cmp_ok( ($edt->replaced_keys('EDT_TEST')), '==', 1, "replaced_keys returns proper count for EDT_TEST" );
    cmp_ok( ($edt->replaced_keys('EDT_AUX')), '==', 2, "replaced_keys returns proper count for EDT_AUX" );
    cmp_ok( ($edt->replaced_keys), '==', 3, "replaced_keys returns proper count for all tables" );
    cmp_ok( ($edt->deleted_keys), '==', 0, "deleted_keys returns empty list" );
    cmp_ok( ($edt->failed_keys), '==', 0, "failed_keys returns empty list" );
    
    # Check that the replaced key lists match up.
    
    my %check_1 = map { $_ => 1 } $edt->replaced_keys;
    my %check_2 = map { $_ => 1 } $edt->replaced_keys('EDT_TEST'), $edt->replaced_keys('EDT_AUX');
    ...
    $result = $edt->replace_record('EDT_AUX', { aux_no => $aux_keys[1], test_no => $test_keys[0],
						name => "replaced again" });
...
subtest 'bad' => sub {
    ...
    eval {
	$edt->replace_record('BAD TABLE', { test_no => '1', string_req => 'test x2' });
    };
    
    ok( $@ && $@ =~ /unknown table/i, "replace_record threw an 'unknown table' exception with bad table name" );
};
...
subtest 'subclass' => sub {
    ...
    # Now do some replaces.
    
    $edt = $T->new_edt($perm_a);
    
    $result = $edt->replace_record('EDT_TEST', { test_no => $keys[0], string_req => 'authorize methods',
						string_val => 'abc' });
    
    ok( $result, "replace_record succeeded" );
    ...
    $result = $edt->replace_record('EDT_TEST', { test_no => $keys[0], string_req => 'validate methods',
						string_val => 'def' });
    ...
    $result = $edt->replace_record('EDT_TEST', { test_no => $keys[0], string_req => 'before methods',
						string_val => 'ghi' });
    ...
    $result = $edt->replace_record('EDT_TEST', { test_no => $keys[0], string_req => 'after methods',
						string_val => 'ghi' });
...
# Now check that a deliberately created error will produce the correct E_EXECUTE condition. We
# cannot test for duplicate key creation, because the semantics of REPLACE actually delete all
# duplicate keys before the insert.
...
subtest 'execution errors' => sub {
    ...
    # $edt = $T->new_edt($perm_a, { IMMEDIATE_MODE => 1, SILENT_MODE => 1, DEBUG_MODE => 1 });
    ...
    # $edt->replace_record('EDT_AUX', { name => 'abc', aux_no => '@x1' });
    ...
    # $T->ok_result($edt->commit, "transaction committed properly");
	...
	$edt->replace_record('EDT_TEST', { test_no => $key, string_req => 'replaced' });

	$T->ok_has_error('E_EXECUTE');
    }
