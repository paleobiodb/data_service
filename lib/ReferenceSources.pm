...
sub metadata_query {
	...
	# Return the status line, the query text, the query url, and the results if any.
	
	if ( $response )
	{
	    my $content = $response->decoded_content;
	    
	    return $status, $query_text, $query_url, $content;
	}
...
sub store_scores {
    ...
    my $sql = "REPLACE INTO $TABLE{REFERENCE_SCORES} (reference_no, refsource_no, rs_index, 
	...
	VALUES ($refno, $sourceno, $idx, $title_s, $title_c, $pub_s, $pub_c, 
	$s->{auth1_s}, $s->{auth1_c}, $s->{auth2_s}, $s->{auth2_c}, $s->{pubyr_s}, $s->{pubyr_c},
	$s->{volume_s}, $s->{volume_c}, $s->{pages_s}, $s->{pages_c}, $pblshr_s, $pblshr_c)";
...
sub recount_scores {
    ...
    $sql = "UPDATE $TABLE{REFERENCE_SCORES} SET
	count_ps = sign(title_s div 100)+sign(pub_s div 100)+sign(auth1_s div 100)+
		sign(auth2_s div 100)+sign(pubyr_s div 100)+sign(volume_s div 100)+
		sign(pages_s div 100)+sign(pblshr_s div 100),
	count_pc = sign(title_c div 100)+sign(pub_c div 100)+sign(auth1_c div 100)+
