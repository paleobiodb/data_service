...
sub establish_procedures {
    ...
    # If we are writing out the procedure definitions to a file, then start by opening that
    # file. The procedures will be written out twice, once for the main database and once for the
    # test database.
	...
	if ( $options->{test_db} )
	{
	    push @databases, $options->{test_db};
	}
...
EndHeader
    ...
    # Otherwise, check if the name of the bounds table is qualified by a database name. If so,
    # execute a 'use' statement so that the procedures will be created in that database.
    ...
    # Now generate statements to create or replace the necessary procedures.
    
    logMessage(1, "Creating or replacing stored procedures...");
    
    foreach my $dbname (@databases)
    {
	logMessage(2, "  Database $dbname:");

	MakeStmt($dest, "use $dbname;");
	
	MakeStmt($dest, "delimiter //");
	
	logMessage(2, "    complete_bound_updates");
	
	MakeStmt($dest, "CREATE OR REPLACE PROCEDURE complete_bound_updates ( )
	BEGIN
	...
	MakeStmt($dest, "CREATE OR REPLACE PROCEDURE update_timescale_ages ( t int unsigned )
	BEGIN
	...
	MakeStmt($dest, "CREATE OR REPLACE PROCEDURE check_updated_timescales ( )
	BEGIN
	...
	MakeStmt($dest, "CREATE OR REPLACE PROCEDURE link_timescale_bounds ( t int unsigned )
    	BEGIN
	...
	MakeStmt($dest, "CREATE OR REPLACE PROCEDURE unmark_updated_bounds ( )
    	BEGIN
	...
	{
	    print STDOUT $SQL_TEXT;
	}
	
	else
	{
	    open(my $outfh, '>', $options->{outfile}) || die "ERROR: Could not write $options->{outfile}: $!\n";
	    print $outfh $SQL_TEXT;
	    close($outfh) || die "ERROR: Could not write $options->{outfile} $!\n";
	}
    }
}
