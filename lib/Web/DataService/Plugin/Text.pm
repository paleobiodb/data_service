...
sub emit_header {
	...
	if ( my @extra = $request->extra_datainfo_keys )
	{
		...
		my $title = $request->extra_datainfo_title($key) || $key;
		
		if ( ref $value && reftype $value eq 'HASH' )
		{
		    $output .= $class->emit_line($request, $title);
		    
		    my @keys;
		    
		    if ( defined $value->{_keys} && ref $value->{_keys} eq 'ARRAY' )
		    {
			@keys = @{$value->{_keys}};
		    }
			...
			{
			    $output .= $class->emit_line($request, '', @{$value->{$k}});
			}
		...
		elsif ( ref $value && reftype $value eq 'ARRAY' )
		{
			...
			if ( ref $row eq 'ARRAY' )
			{
			    $output .= $class->emit_line($request, '', @$row);
			}
		...
		{
		    $output .= $class->emit_line($request, $title, $value);
		}
	    }
	}
    }
