...
sub update_timescales {
    ...
    # Now go through the records and validate them one by one.
    
    foreach my $r ( @records )
    {
	my $record_id = $r->{record_id} || $r->{bound_id} || '';
	
	my $result = $request->validate_params('1.2:timescales:entry', $r);
	...
	foreach my $r ( @records )
	{
	    my $record_id = $r->{record_id} || $r->{timescale_id} || '';
	    
	    if ( $r->{timescale_id} )
	    {
		print STDERR "UPDATING record '$record_id'\n" if $request->debug;
		
		$edt->update_timescale($r, \%conditions);
	    }
		...
		$edt->add_timescale($r, \%conditions);
	    }
	...
	$request->list_timescales_for_update($dbh, $return_what, $list) if $list;
    }
}
...
sub delete_timescales {
	...
	if ( $list )
	{
	    $edt->delete_timescales($list, \%conditions);
	}
...
sub update_bounds {
    ...
    my $edt = TimescaleEdit->new($dbh, { debug => $request->debug,
					 session_id => Dancer::cookie('session_id') });
