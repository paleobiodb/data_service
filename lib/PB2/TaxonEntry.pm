...
sub initialize {
	...
	{ select => ['pc.orig_no', 't.name as taxon_name', 't.trad_rank as taxon_rank',
		     't.status', 't.accepted_no', 't2.name as accepted_name',
	...
	{ output => 'accepted_no', com_name => 'acc', dedup => 'orig_no' },
	    "If this taxon is invalid or a junior synonym, the identifier",
    ...
    $ds->define_block('1.2:taxa:choice_image' =>
	{ set => '*', code => \&PB2::TaxonData::process_image_ids },
	...
	{ output => 'created', com_name => 'dcr' },
	    "The date and time at which this image was created on phylopic.org",
	{ output => 'modified', com_name => 'dmd' },
	...
	{ output => 'attribution', com_name => 'att', dedup => 'credit' },
	    "The attribution of this image, if different from the value of C<B<credit>>.",
...
sub list_image_choices {
    ...
    my $show_new = $request->clean_param('new');
    ...
    # Generate the main queries. The first one returns taxonomic concepts with lists of
    # image choices, the second one returns data about the images themselves.
    
    my ($choice_sql, $image_sql);
    
    if ( $show_new )
    {
	my $new_clause = $show_new eq 'me' ? "and ps.person_no = '$perms->{enterer_no}'" : '';
	...
	$choice_sql = "SELECT $fields, group_concat(pn.image_no) as choices
		FROM $TABLE{PHYLOPIC_CHOICE} as pc
		    ...
		    join (SELECT distinct orig_no FROM $TABLE{TAXON_TREES} as t
				...
				on ps.image_no = pn.image_no and ps.orig_no = pn.orig_no $new_clause
    ...
    $request->debug_line("$choice_sql\n\n") if $request->debug;
    
    my $choice_result = $dbh->selectall_arrayref($choice_sql, { Slice => {} });
    
    $request->debug_line("$image_sql\n\n") if $request->debug;
    
    my $image_result = $dbh->selectall_arrayref($image_sql, { Slice => {} });
    
    # Each record from the image result should be output using the block
    # '1.2:taxa:choice_image'.
    
    if ( $image_result )
    {
	foreach my $r ( $image_result->@* )
	{
	    $request->select_record_output($r, '1.2:taxa:choice_image');
	}
    }
    
    # Return both result lists, in the order they were generated.
    
    $request->list_result($choice_result, $image_result);
}
