...
sub rebuildCache {
    ...
    if (!$list_only)  {
        ...
        $result = $dbh->do("CREATE TABLE ${TAXA_TREE_CACHE}_new (taxon_no int(10) unsigned NOT NULL default '0',lft int(10) unsigned NOT NULL default '0',rgt int(10) unsigned NOT NULL default '0', spelling_no int(10) unsigned NOT NULL default '0', synonym_no int(10) unsigned NOT NULL default '0', opinion_no int(10) unsigned NOT NULL default '0', max_interval_no int(10) unsigned NOT NULL default '0', min_interval_no int(10) unsigned NOT NULL default '0', mass float default NULL, PRIMARY KEY  (taxon_no), KEY lft (lft), KEY rgt (rgt), KEY spelling_no (spellign_no), KEY synonym_no (synonym_no), KEY opinion_no (opinion_no)) TYPE=MyISAM");
...
# JA: these two venerable Schroeter functions are used by taxa_cached.pl
#  to figure out which opinions are new, so don't touch them
sub getSyncTime {
    my $dbt = shift;
...
# JA: there was a horrible bug somewhere in this routine that caused sporadic
#  table corruptions when opinions were being entered rapidly, which I first
#  figured out on 28.11.07; on 24.4.13 I simplified it to make simultaneous
