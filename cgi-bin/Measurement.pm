...
sub displaySpecimenList {
    my ($dbt,$taxonomy,$hbo,$q,$s,$called_from,$more_links) = @_;
...
|;
	displaySpecimenList($dbt,$taxonomy,$hbo,$q,$s,'processMeasurementForm',$more_links);
	if ($q->param('occurrence_no')) {
...
sub displayDownloadMeasurementsResults  {
	...
	my $options = { get_global_specimens => 1};
	...
	my @measurements = getMeasurements($dbt, $taxonomy, $options);
