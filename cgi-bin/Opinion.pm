...
sub displayOpinionForm {
	...
	if (! $reSubmission) {
        if ($isNewEntry) {
            ...
            #  "belongs to," original spelling, authoritative, and with a new
            #  diagnosis JA 6.6.07
            my $sql = "SELECT diagnosis_given,ref_has_opinion,o.pages,classification_quality FROM opinions o,authorities a WHERE child_no=taxon_no AND ((diagnosis_given!='' AND diagnosis_given IS NOT NULL) OR (o.pages!='' AND o.pages IS NOT NULL) OR (classification_quality!='' AND classification_quality IS NOT NULL)) AND o.reference_no=" . $s->get('reference_no') . " AND o.enterer_no=" . $s->get('enterer_no') . " AND child_no!=" . $q->param('child_no') . " AND (taxon_rank NOT LIKE '%species%' OR status!='belongs to' OR spelling_reason!='original spelling' OR classification_quality!='authoritative' OR diagnosis_given!='new') ORDER BY opinion_no DESC LIMIT 1";
    ...
    if ($fields{'parent_spelling_no'} > 0) {
        ...
        if ($fields{'taxon_status'} eq 'belongs to' && $q->param('belongs_to_parent')) { 
            $parentName = $q->param('belongs_to_parent');
        } elsif ($fields{'taxon_status'} eq 'invalid1' && $q->param('synonym_parent')) { 
